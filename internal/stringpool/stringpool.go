...
// cp is not implemented or any existing string contains a rune not
// representable in cp.
...
// Len returns the number of entries in the pool, including released
// slots that remain reserved by ID.
...
// Intern returns the ID for s, incrementing its persistent or non-persistent
// refcount. The empty string returns 0. Returns an error if s contains a rune 
// not representable in the pool's current code page.
func (p *Pool) Intern(s string, persistent bool) (uint32, error) {
	if s == "" {
		return 0, nil
	}
	if id, ok := p.index[s]; ok {
		...
		switch {
		case !persistent:
			if e.nonpersistentRefcount < maxRefcount {
				e.nonpersistentRefcount++
			}
		case e.persistentRefcount == 0:
			if err := p.validate(s); err != nil {
				return 0, err
			}
			e.persistentRefcount = 1
		case e.persistentRefcount < maxRefcount:
...
// validate reports whether s encodes in the pool's current code page.
